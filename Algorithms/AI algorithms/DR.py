...
def PCA(X, out_dim):
    ...
    # Compute the mean vector (D-by-1)
    mu = np.mean(X, axis=1, keepdims=True) #Each dimension have mean now.
    
    # Center the data by subtracting the mean
    X_centered = X - mu #Now we get mean-centered data, where each feature has a mean of zero
    
    # Compute the covariance matrix (D-by-D)
    Sigma = np.matmul(X_centered, X_centered.T) / (N - 1) 
    #To get sigma, we should multiply X_centered matrix with transposed matrix itslef and divide it by N-1
    ...
    """
    V, W = np.linalg.eigh(Sigma) #V is eigenvalue and W is eigenvector
...
def main(args):
    ...
    if args.method == "PCA":
        ...
        new_X = np.matmul(W.T, X - mu) #Compute dot product 
