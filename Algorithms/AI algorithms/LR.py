...
def linear_regression(X, Y):
    ...
    A_T = A.transpose()  # A^T
    A_T_A_inv = np.linalg.inv(np.matmul(A_T, A))  # (A^T A)^(-1)
    A_T_Y = np.matmul(A_T, Y)  # A^T Y
    p = np.matmul(A_T_A_inv, A_T_Y)  # Final parameter vector
    ### Your job ends here ###
    return p
...
def polynomial_regression(X, Y, degree):
    ...
    N = X.shape[0]
    A = np.ones((N, degree + 1))  # Initialize A matrix with 1s for the bias term

    for d in range(1, degree + 1):
        A[:, d] = X[:, 0] ** d  # Broadcasting to calculate each polynomial degree
